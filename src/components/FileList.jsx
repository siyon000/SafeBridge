...
const FileList = ({ files, progress = {} }) => {
    return (
        <ul className="divide-y divide-gray-700 border border-gray-700 rounded-lg overflow-hidden">
            {files.map(file => (
                <li key={file.id || file.name} className="p-3 bg-gray-800">
                        ...
                        {file.done && (
                            <div className="mt-2 sm:mt-0 sm:ml-4">
                                <a
                                    ...
                                    download={file.name}
                                    className="inline-flex items-center justify-center bg-blue-900 text-blue-200 px-3 py-1 rounded-full text-sm hover:bg-blue-800 transition-colors"
                                    title="Download"
                                    aria-label={`Download ${file.name}`}
                                >
                                    <svg className="h-4 w-4 mr-1 text-blue-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
